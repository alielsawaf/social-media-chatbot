...
def get_answer(text):
    ...
    # 3. قسم البطارخ (عشان نفرق بين البوري والرنجة)
    if "بطارخ" in q:
        if "بوري" in q or "نشي" in q or "نشو" in q:
            return f"💰 {PRODUCT_MAP['Herring Roe White']}" # ده سعر بطارخ البوري (النشو)
        return f"💰 أسعار بطارخ الرنجة:\n- {PRODUCT_MAP['Herring Roe with Vegetable Oil']}\n- {PRODUCT_MAP['Herring Roe with Orange Sauce']}"

    # 4. قسم الرنجة (دقة الفيليه والـ 24)
    if "رنج" in q:
    ...
    # 5. قسم الفسيخ والماكريل والتونة
    ...
    if "تون" in q:
        if any(w in q for w in ["مصر", "مستورد", "نوع"]): return FAQ_MAP["التونة مستوردة ولا مصري"]
        return "💰 تونة أبو السيد مصري يلوفين، تبدأ من 70 ج. تحب أبعتلك المنيو؟"

    # 6. حل مشكلة "اه" (التأكيد الذكي)
