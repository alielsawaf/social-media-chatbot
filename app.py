...
# ================== DATA ==================
# (ملاحظة: تركت الخرائط كما هي في كودك الأصلي تماماً لضمان عدم ضياع البيانات)
FAQ_MAP = {
...
PRODUCT_MAP = {
    # (نفس بيانات المنتجات التي أرفقتها)
...
def normalize(text):
    ...
    # حذف علامات الترقيم (مهم جداً عشان "سندوتش؟" تبقى "سندوتش")
...
def get_answer(text):
    ...
    # 1. الترحيب والشكر
    if any(w in q for w in ["اهلا", "سلام", "مساء", "صباح", "مرحبا", "هاي", "ازيك"]):
    ...
    # 2. الشكاوي (أولوية قصوى)
    if any(w in q for w in ["دود", "مدود", "طفيليات", "حشره"]):
        return FAQ_MAP["الرنجة فيها دود"]
    if "دم" in q:
        return FAQ_MAP["ليه الفسيخ بيكون في دم"]

    # 3. الساندوتشات والسلطة (حل المشكلة اللي واجهتك)
    if any(w in q for w in ["سندوتش", "ساندوتش", "سلطه"]):
        return FAQ_MAP["السندوتشات"]

    # 4. المنيو العام
    if any(w in q for w in ["منيو", "كتالوج", "قائمه", "ليست"]):
        return f"ده لينك منيو المنتجات بتاعتنا كاملة بالأسعار:\n{FAQ_MAP['منيو']}"

    # 5. ذكاء المنتجات (فسيخ وبوري)
    if any(w in q for w in ["فسيخ", "بوري", "ملوح"]):
        ...
        if "ملح" in q: return FAQ_MAP["الفسيخ بيتملح ازاي"]
        return (
            ...
            "للمزيد من الأنواع ابعتلنا اسم الصوص المفضل (كاري، بنجر، فلفل)."
        )

    # 6. رنجة
    if "رنج" in q:
        if "فيليه" in q:
            if "ناشف" in q: return FAQ_MAP["ليه الرنجة الفيليه ناشفة"]
            return PRODUCT_MAP["Herring Fillets without Oil"]
        if "24" in q or "قيراط" in q: return FAQ_MAP["الفرق بين الرنجة العادية وعيار 24"]
        if "فاكيوم" in q: return FAQ_MAP["يعني ايه رنجة فاكيوم"]
        if "مبطرخ" in q: return PRODUCT_MAP["Smoked Herring with Roe"]
        if "اشوي" in q or "نار" in q: return FAQ_MAP["ممكن اشوي الرنجة"]
        return (
            ...
            "لو حابب أنواع الفاكيوم أو الفيليه المخلية قولي."
        )

    # 7. معلومات التواصل والإدارة
    if "مواعيد" in q or "مفتوح" in q: return FAQ_MAP["مواعيد الفروع"]
    if "تصدير" in q: return FAQ_MAP["التصدير"]
    if "توظيف" in q or "شغل" in q: return FAQ_MAP["التوظيف"]
    if "جمله" in q: return FAQ_MAP["الجملة او اسعار الجملة"]
    if "غالي" in q: return FAQ_MAP["ليه المنتجات غالية"]

    return "بعتذر لحضرتك يافندم.. ممكن توضح السؤال أكتر (مثلاً: اسعار الرنجة، مواعيد الفروع، منيو الفسيخ) عشان أقدر أساعدك؟"
