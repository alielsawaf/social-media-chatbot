...
def get_answer(text):
    ...
    # 1. الترحيب الصريح (لو النص فيه كلمة ترحيب فقط أو بيبدأ بيها)
    if q in ["اهلا", "سلام", "لو سمحت", "مرحبا", "هاي"]:
        return "أهلاً بك في رنجة أبو السيد 👋 نورتنا.. أساعد حضرتك ازاي؟"

    # 2. الأولوية القصوى للـ FAQ (الأسئلة الإجابة عليها نصية)
    # ده هيحل مشكلة "الفرق بين 24 والعادية" و "التونة مصري"
    for key, value in FAQ_MAP.items():
        if normalize(key) in q:
            return value

    # 3. معالجة طلب الأسعار (لما يكتب سعر + اسم صنف)
    # هنحول كلمة سعر وما بعدها لرد مباشر
    if "سعر" in q or "بكام" in q:
        if "فسيخ" in q:
            return f"{PRODUCT_MAP['Salted Mullet without Bacteria']}\n\n{PRODUCT_MAP['Salted Mullet with Roe']}"
        if "رنج" in q: # كلمة رنج عشان تجيب رنجة ورنجة
            return f"{PRODUCT_MAP['Smoked Herring']}\n\n{PRODUCT_MAP['Smoked Herring 24 Kerat']}"
        if "تون" in q:
            return "💰 تونة أبو السيد (يلوفين) متوفرة قطع ومفتتة بأسعار تبدأ من 70 ج، تحب أبعتلك المنيو بالتفصيل؟"
        if "ماكريل" in q:
            return f"{PRODUCT_MAP['Gutted Smoked Mackerel Salted']}\n\n{PRODUCT_MAP['Mackerel Fillets Vacuumed']}"
        if "بطارخ" in q:
            return f"{PRODUCT_MAP['Herring Roe with Vegetable Oil']}\n\n{PRODUCT_MAP['Herring Roe White']}"
        
        # لو كتب "سعر" بس من غير صنف
        return "حضرتك تحب تعرف سعر أنهي منتج؟ (رنجة – فسيخ – ماكريل – سلمون – تونة)"

    # 4. معالجة الكلمات المفردة (لو العميل كتب "تونة" بس)
    if "تون" in q: return "التونة عندنا يلوفين ومصيدة من البحر المتوسط، متوفرة قطع وسبريد. تحب تعرف أسعارها؟"
    if "ماكريل" in q: return "عندنا ماكريل مدخن ومملح (سادة وبالتوابل) فاكيوم. تحب تعرف أسعاره؟"
    if "بطارخ" in q: return "متاح بطارخ رنجة (مرملة كاملة) وبطارخ بوري (نشو). تحب تعرف أسعارها؟"
    if "فسيخ" in q: return f"{PRODUCT_MAP['Salted Mullet without Bacteria']}\n\n{PRODUCT_MAP['Salted Mullet with Roe']}"
    if "رنج" in q: return f"{PRODUCT_MAP['Smoked Herring']}\n\n{PRODUCT_MAP['Smoked Herring 24 Kerat']}"

    # 5. الرد الافتراضي المحسن
    return "ممكن حضرتك توضح استفسارك أكتر؟ (مثلاً: سعر الرنجة، هل التونة مصري؟، أماكن الفروع)"
  # ================== WEBHOOK ==================
