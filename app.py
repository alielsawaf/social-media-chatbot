...
def get_answer(text):
    ...
    # 1. الترحيب (دايماً في الأول)
    if any(w in q for w in ["اهلا", "سلام", "ازيك", "نورتنا"]):
        return "أهلاً بك في رنجة أبو السيد 👋 نورتنا.. أساعد حضرتك ازاي؟"

    # 2. البحث عن الأسئلة الطويلة (FAQ) - ليها الأولوية القصوى
    # هنا بنلف على الأسئلة الطويلة الأول عشان لو العميل سأل "ايه الفرق بين رنجة 24 والعادية"
    # يجاوبه بالفرق مش بالسعر.
    for key in FAQ_MAP:
        if normalize(key) in q:
            return FAQ_MAP[key]

    # 3. الكلمات المفتاحية الحرجة (دود، دم، تصدير، توظيف)
    # بنعمل قاموس صغير للحالات دي
    critical_keywords = {
        "دود": "الرنجة فيها دود",
        "دم": "ليه الفسيخ بيكون في دم",
        "تصدير": "التصدير",
        "توظيف": "التوظيف",
        "شغل": "التوظيف",
        "منيو": "منيو"
    }
    for word, map_key in critical_keywords.items():
        if word in q:
            return FAQ_MAP[map_key]

    # 4. أسعار محددة جداً (عيار 24)
    if "24" in q:
        return PRODUCT_MAP["Smoked Herring 24 Kerat"]

    # 5. الفئات العامة (فسيخ، رنجة، ماكريل)
    if "فسيخ" in q:
        return f"{PRODUCT_MAP['Salted Mullet without Bacteria']}\n\n{PRODUCT_MAP['Salted Mullet with Roe']}"
    
    if "رنجه" in q: # الـ normalize هيحول رنجة لـ رنجه
        return f"{PRODUCT_MAP['Smoked Herring']}\n\n{PRODUCT_MAP['Smoked Herring 24 Kerat']}"

    # 6. طلب الأسعار العام
    if any(w in q for w in ["اسعار", "سعر", "بكام", "قائمه"]):
