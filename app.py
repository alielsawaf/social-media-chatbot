...
# --- 1. قاعدة بيانات المنتجات (الصنف، السعر، الوزن) ---
PRODUCTS = [
    # الرنجة
...
def get_answer(user_question):
    q = clean_arabic_text(user_question)
      # 1. التحقق من وجود "سعر" أو "بكام" + "اسم صنف"
    price_words = ['بكام', 'سعر', 'اسعار', 'قيمه', 'تكلفه','في','عندكو','عندكم','موجود','فيه','بتبيعو']
    is_asking_price = any(pw in q for pw in price_words)
    #  البحث في المنتجات
    for prod in PRODUCTS:
        for kw in prod['kw']:
            if clean_arabic_text(kw) in q:
                return f"طلب حضرتك متاح: {prod['kw'][0]}\n💰 السعر: {prod['price']}\n⚖️ الوزن: {prod['w']}\n\nتفضل المنيو الكامل: {MENU_LINK}"
   
    # 12. إذا سأل "بكام" أو "سعر" بشكل عام (بدون تحديد صنف)
    if is_asking_price or any(kw in q for kw in ['منيو', 'قائمه', 'اصناف','الاسعار']):
        return f"أهلاً بك! تفضل المنيو المحدث بجميع منتجاتنا من هنا:\n{MENU_LINK}"
    ...
    if any(word in q for word in ['سلام', 'هلو', 'ازيك', 'صباح', 'مساء']):
        return "أهلاً بك في أبو السيد، كيف يمكننا مساعدتك ؟"

    # 4. الرد الافتراضي
    return f"عذراً، لم أفهم طلبك بدقة. يمكنك الاطلاع على المنيو بجميع الأسعار من هنا:\n{MENU_LINK}\nأو التواصل مع أحد ممثلي خدمة العملاء عن طريق الواتساب: https://wa.me/{WHATSAPP_NUMBER} \n أو يمكنك السؤال بشكل أوضح حتي أستطيع مساعدتك"
