...
def get_answer(user_id, text):
    ...
    # التعامل مع الفئات والمنتجات
    all_categories = ["رنج", "فيليه", "فسيخ", "بوري", "ماكريل", "بطارخ", "سبريد", "تون", "تونه", "سلمون"]
    ...
    # التعامل مع منتج محدد
    all_products = PRODUCT_MAP.keys()
    ...
    # لو العميل طلب "سعر" فقط بعد حديث سابق عن منتج أو فئة
    if any(w in q for w in ["سعر", "كام", "بكام"]):
        if ctx.get("last_product"):
            return PRODUCT_MAP.get(ctx["last_product"], f"💰 سعر المنتج {ctx['last_product']} مش متوفر حالياً.")
        if ctx.get("last_category"):
            # عرض كل أسعار الفئة
            cat = ctx["last_category"]
            prices = []
            for k, v in PRODUCT_MAP.items():
                if cat in normalize(k):
                    prices.append(v)
            if prices:
                return f"💰 تشكيلة {cat}:\n" + "\n".join(prices)
            else:
                return f"لا توجد منتجات متاحة ضمن {cat} حالياً."

    # التعامل مع أسئلة متعلقة بالفرق أو الخصائص
    ...
    # باقي الأسئلة العامة
    # (هنا نفس الكود القديم بدون تعديل، لأنه يحافظ على كل الأسئلة والمنتجات)
    # مثال: التخزين، الشوي، الوزن، الفسيخ، البطاريخ، التونة، الإدارة، الفروع، الساندويتشات، المواد الحافظة

    # =====================================
    # التعامل مع المنتجات والفئات
    if selected_product:
    ...
    if selected_category:
        prices = []
        for k, v in PRODUCT_MAP.items():
            if selected_category in normalize(k):
                prices.append(v)
        if prices:
            return f"💰 تشكيلة {selected_category}:\n" + "\n".join(prices)
