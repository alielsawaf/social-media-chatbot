...
GENERAL_TRIGGERS = [
    "منيو", "المينيو", "عايز المنيو", "ابعت المنيو",
    "بتبيعو ايه", "ايه المنتجات", "ايه اللي عندكم",
    "من فضلك", "لو سمحت", "ابعت"
]
...
def get_answer(user_text):
    ...
    # ---------- منيو / عام ----------
    for phrase in GENERAL_TRIGGERS:
        if similarity(q_original, clean_arabic_text(phrase)) >= FUZZY_THRESHOLD:
            return f"📖 تفضل المنيو الكامل:\n{MENU_LINK}"

    if any(w in q_original for w in PRICE_WORDS) and len(q_original.split()) <= 2:
    ...
    for p in PRODUCTS:
        for kw in p['kw']:
            if similarity(q_product, clean_arabic_text(kw)) >= FUZZY_THRESHOLD:
                matches.append(p)
                break
...
for item in FAQ:
    ...
    if any(w in q_original for w in ['اهلا', 'سلام', 'هاي', 'ازيك']):
        return "أهلاً بحضرتك 👋 تحب أساعدك في ايه؟"

    # ---------- فشل ----------
    log_failed(user_text)
    return (
        "معلش يا فندم، مش فاهم طلبك قوي.\n"
        f"📖 المنيو الكامل:\n{MENU_LINK}\n"
...
# ================== Endpoint لتحميل CSV ==================
CSV_PASSWORD = "123321"  # ضع هنا كلمة السر الخاصة بك

@app.route('/download_csv', methods=['GET'])
def download_csv():
    password = request.args.get("password")
    if password != CSV_PASSWORD:
        return abort(403, "غير مصرح بالدخول")
    
    if not os.path.isfile(CSV_FILE):
        return "ملف CSV مش موجود بعد"
