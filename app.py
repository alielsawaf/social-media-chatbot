...
# --------------------------------------------------------------------------------
# 🚨 المسار الجديد للتحقق من Webhook (يستقبل GET)
# --------------------------------------------------------------------------------
@app.route('/webhook', methods=['GET'])
def webhook_verification():
    # ⚠️ يجب تغيير هذا التوكن ليتطابق مع التوكن الذي وضعته في منصة المراسلة ⚠️
    VERIFY_TOKEN = "161019" 
    
    # استخراج البارامترات من طلب GET
    mode = request.args.get("hub.mode")
    token = request.args.get("hub.verify_token")
    challenge = request.args.get("hub.challenge")
    
    # التحقق من البارامترات
    if mode and token:
        if mode == "subscribe" and token == VERIFY_TOKEN:
            # نجاح التحقق: الرد بقيمة challenge
...
# --------------------------------------------------------------------------------
# 🚨 المسار الجديد لتلقي الرسائل الواردة (يستقبل POST)
# --------------------------------------------------------------------------------
@app.route('/webhook', methods=['POST'])
def webhook_inbound():
    data = request.get_json()
    
    # هنا يجب أن تستخلص رسالة العميل من الـ data المرسلة من منصة المراسلة
    # بما أننا لا نعرف شكل الـ JSON بالتحديد، سنكتفي بالرد برسالة النجاح فقط لتجنب تعليق الويب هوك.
    
    # يمكنك وضع الكود الخاص بـ get_answer هنا لاحقاً عندما تعرف شكل البيانات:
    # user_message = استخلاص_الرسالة(data) 
    # result = get_answer(user_message)
    # رد_على_منصة_المراسلة(result['answer'])

    # الرد يجب أن يكون دائماً 200 OK لمنصة المراسلة حتى لا يتم إيقاف الـ webhook
    return "EVENT_RECEIVED", 200 

# --------------------------------------------------------------------------------
# مسار /ask القديم للاختبار اليدوي (يمكن تركه أو حذفه)
# --------------------------------------------------------------------------------
