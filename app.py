...
def get_answer(user_id, text):
    ...
    # 1. كلمات التأكيد (عشان يفهم "آه" و "ماشي")
    CONFIRMATION_WORDS = ['اه', 'ايوه', 'ماشي', 'طيب', 'قولي', 'وريني', 'تمام', 'قول']
    
    # 2. تحديث الذاكرة (الأولوية لاسم المنتج قبل أي شيء)
    ...
    if any(w in raw_q for w in ['اهلا', 'سلام', 'صباح', 'مساء', 'ازيك']):
        return "أهلاً بك في رنجة أبو السيد 👋 نورتنا.. حابب تعرف الأسعار ولا عندك استفسار؟"

    # 4. لو العميل قال "آه" أو "بكام"
    is_price_req = any(x in raw_q for x in ["سعر", "بكام", "كام", "قد ايه", "جنيه", "بكم"])
    is_confirm = any(c == q_cleaned for c in CONFIRMATION_WORDS) # فحص كلمة "اه" لوحدها
    ...
    # 5. الـ FAQ (لازم نتأكد إن الجملة مش اسم منتج قبل ما نرد من هنا)
    if not product: # لو ملقيناش منتج، دور في الـ FAQ
        for f in FAQ:
            for kw in f["keywords"]:
                if fuzz.partial_ratio(q_cleaned, clean(kw)) > 90: # عليت الدقة هنا عشان ميتلخبطش
                    return f["answer"]
    ...
    return "بعتذر منك يا فندم، ممكن توضحلي استفسارك أكتر. ✨"
# ================== WEBHOOK ==================
