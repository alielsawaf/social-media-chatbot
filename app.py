...
def get_answer(text):
    ...
    # 1. الترحيب والذوقيات (حل مشكلة شكراً وتمام)
    greetings = ["اهلا", "سلام", "مساء", "صباح", "مرحبا", "هاي", "ازيك"]
    thanks = ["شكرا", "تمام", "ميرسي", "تسلم", "جزاك الله", "ماشي"]
    
    if any(word in q for word in thanks):
        return "تحت أمرك يا فندم في أي وقت! نورتنا في رنجة أبو السيد. 🌹"
    ...
    if "منيو" in q or "كتالوج" in q:
        return f"ده لينك منيو المنتجات بتاعتنا كاملة بالأسعار: {FAQ_MAP['منيو']}"

    # 2. قسم "المشاكل والشكاوي" (الأولوية المطلقة)
    # حطينا ده قبل الأسعار عشان لو قال "الفسيخ فيه دم" يرد على الدم مش السعر
    if "دود" in q or "طفيليات" in q: return FAQ_MAP["الرنجة فيها دود"]
    if "دم" in q: return FAQ_MAP["ليه الفسيخ بيكون في دم"]
    
    # 3. قسم "ليه" و "ايه الفرق"
    if any(w in q for w in ["ليه", "سبب", "فرق", "الفرق", "ازاي", "ازي"]):
        if "ناشف" in q or "فيليه" in q: return FAQ_MAP["ليه الرنجة الفيليه ناشفة"]
        ...
        if "اصليه" in q or "اعرف" in q: return FAQ_MAP["ازاي اتأكد ان الرنجة دي رنجة ابو السيد"]

    # 4. قسم عرض كل الأسعار المتعلقة (Comprehensive Prices)
    
    # --- كل ما يخص الرنجة ---
    if "رنج" in q:
        # لو حدد فيليه نجيب الفيليه بس، لو رنجة عامة نجيب كله
        if "فيليه" in q:
            return f"💰 أسعار الفيليه والصوصات:\n- {PRODUCT_MAP['Herring Fillets without Oil']}\n- {PRODUCT_MAP['Herring Fillets with Pepper Sauce']}\n- {PRODUCT_MAP['Herring Fillets with Curry Sauce']}"
        return (f"💰 تشكيلة الرنجة عندنا:\n"
                ...
                f"تحب تعرف أسعار الفيليه والصوصات كمان؟")

    # --- كل ما يخص الفسيخ ---
    if "فسيخ" in q or "بوري مملح" in q:
        return (f"💰 أسعار الفسيخ والبوري:\n"
    ...
    # --- كل ما يخص البطارخ ---
    if "بطارخ" in q:
        return (f"💰 أنواع البطارخ المتاحة:\n"
                f"- بطارخ بوري (نشو): {PRODUCT_MAP['Herring Roe White']}\n"
                f"- بطارخ رنجة مرملة كاملة: {PRODUCT_MAP['Herring Roe with Vegetable Oil']}\n"
                f"- بطارخ رنجة بصوص البرتقال: {PRODUCT_MAP['Herring Roe with Orange Sauce']}\n"
                f"- بطارخ رنجة بصوص العسل: {PRODUCT_MAP['Herring Roe with Honey Sauce']}")

    # 5. التونة والماكريل والسردين
    if "ماكريل" in q:
        return f"💰 أسعار الماكريل:\n- {PRODUCT_MAP['Gutted Smoked Mackerel Salted']}\n- {PRODUCT_MAP['Mackerel Fillets Vacuumed']}"
    if "تون" in q:
        if any(w in q for w in ["مصر", "مستورد", "نوع", "منين"]): return FAQ_MAP["التونة مستوردة ولا مصري"]
        return "💰 تونة أبو السيد مصري يلوفين، تبدأ من 70 ج. (قطع - مفتتة - سبريد). تحب أبعتلك المنيو؟"
    if "سردين" in q: return "للأسف السردين غير متاح حالياً، متوفر فسيخ بوري ممتاز، تحب أعرضلك أسعاره؟"

    # 6. الردود الذكية الأخرى
    if q in ["اه", "ايوه", "ياريت", "تمام", "ماشي"]:
        return "من عيوني! قولي محتاج تعرف أسعار إيه بالظبط؟ رنجة ولا فسيخ ولا بطارخ؟"
