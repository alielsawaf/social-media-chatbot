...
def get_answer(text):
    ...
    # 1. كلمات التأكيد (حل مشكلة "اه ياريت" و "ابعت")
    # لو العميل وافق على عرض سعر بعد ما البوت اقترح عليه
    if q in ["اه", "ياريت", "ايوه", "تمام", "ماشي", "قول", "ابعت", "اوك", "اوكي"]:
        return "من عيوني! قولي تحب تعرف أسعار (الرنجة) ولا (الفسيخ) ولا (الماكريل)؟"

    # 2. الكلمات الحساسة والأسئلة (الأولوية القصوى)
    if "دود" in q or "طفيليات" in q: return FAQ_MAP["الرنجة فيها دود"]
    if "دم" in q: return FAQ_MAP["ليه الفسيخ بيكون في دم"]
    
    # 3. الأسئلة عن الفرق (لازم تكون قبل الأسعار)
    ...
    # 4. تدقيق الأسعار (Specific Products)
    # هنا بندور على "سعر + النوع" بدقة قبل ما نرد بالرد العام
    if "سعر" in q or "بكام" in q or "اسعار" in q:
        if "فيليه" in q:
            if "رنج" in q: return PRODUCT_MAP["Herring Fillets without Oil"]
            if "ماكريل" in q: return PRODUCT_MAP["Mackerel Fillets Vacuumed"]
        ...
        if "ماكريل" in q: return PRODUCT_MAP["Gutted Smoked Mackerel Salted"]
        if "تون" in q: return "💰 تونة أبو السيد (يلوفين) مصري، تبدأ من 70 ج. تحب أبعتلك المنيو؟"

    # 5. التعامل مع التونة (سؤال مصري ولا مستورد)
    if "تون" in q:
        if any(w in q for w in ["مصر", "مستورد", "نوع"]):
            return FAQ_MAP["التونة مستوردة ولا مصري"]
        return "التونة عندنا يلوفين ومصيدة من البحر المتوسط. تحب تعرف أسعارها؟"

    # 6. الردود العامة (لو ملقاش سعر محدد)
    if "رنج" in q: return "عندنا رنجة عادية، وعيار 24، وفيليه سادة أو بصوصات.. تحب أعرضلك الأسعار؟"
    if "ماكريل" in q: return "عندنا ماكريل مدخن ومملح (سادة وبالتوابل). تحب تعرف أسعاره؟"

    # 7. الرد الافتراضي
    return "ممكن حضرتك توضح استفسارك أكتر؟ (مثلاً: سعر الرنجة، الفروع، أو الفرق بين أنواع الرنجة)"  # ================== WEBHOOK ==================
