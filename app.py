...
def get_answer(text):
    ...
    # 1. الترحيب (كلمة واحدة فقط)
    if q in ["اهلا", "سلام", "مرحبا", "هاي"]:
        return "أهلاً بك في رنجة أبو السيد 👋 نورتنا.. أساعد حضرتك ازاي؟"

    # 2. الكلمات "الحساسة" جداً (الأولوية رقم 1)
    # لو الجملة فيها الكلمات دي، انسى أي حاجة تانية ورد بالرد ده
    if "دود" in q or "طفيليات" in q:
        return FAQ_MAP["الرنجة فيها دود"]
    if "دم" in q:
        return FAQ_MAP["ليه الفسيخ بيكون في دم"]
    if "فرق" in q or "الفرق" in q:
        # هنا بنحدد الفرق بين إيه بالظبط
        ...
        if "تونه" in q or "تونا" in q: return FAQ_MAP["الفرق بين لحم التونة الابيض والاحمر"]
        if "فسيخ" in q: return FAQ_MAP["الفرق بين الكيس الاسود والجولد فاكيوم في الفسيخ"]

    # 3. الأسئلة العامة (FAQ) - الأولوية رقم 2
    for key, value in FAQ_MAP.items():
        if normalize(key) in q:
            return value

    # 4. طلب الأسعار (سعر + صنف) - الأولوية رقم 3
    if any(w in q for w in ["سعر", "بكام", "قائمه", "منيو"]):
        if "رنج" in q: return f"{PRODUCT_MAP['Smoked Herring']}\n\n{PRODUCT_MAP['Smoked Herring 24 Kerat']}"
        if "فسيخ" in q: return f"{PRODUCT_MAP['Salted Mullet without Bacteria']}\n\n{PRODUCT_MAP['Salted Mullet with Roe']}"
        if "ماكريل" in q: return f"{PRODUCT_MAP['Gutted Smoked Mackerel Salted']}\n\n{PRODUCT_MAP['Mackerel Fillets Vacuumed']}"
        if "تون" in q: return "💰 تونة أبو السيد (يلوفين) مصري من البحر المتوسط، السعر يبدأ من 70 ج."
        if "بطارخ" in q: return f"{PRODUCT_MAP['Herring Roe with Vegetable Oil']}\n\n{PRODUCT_MAP['Herring Roe White']}"
        return "تحب تعرف سعر أنهي منتج؟ (رنجة – فسيخ – ماكريل – سلمون – تونة)"

    # 5. الأصناف (كلمة واحدة فقط) - الأولوية رقم 4
    if "رنج" in q: return "عندنا رنجة عادية، وعيار 24، وفيليه سادة أو بصوصات.. تحب أعرضلك الأسعار؟"
    if "فسيخ" in q: return "فسيخ أبو السيد متوفر بوري سليم وفيليه وسبريد.. تحب أعرضلك الأسعار؟"
    if "ماكريل" in q: return "عندنا ماكريل مدخن ومملح (سادة وبالتوابل) فاكيوم. تحب تعرف أسعاره؟"
    if "تون" in q: return "التونة عندنا يلوفين ومصيدة من البحر المتوسط. تحب تعرف أسعارها؟"

    # 6. الرد الافتراضي
    return "ممكن حضرتك توضح استفسارك أكتر؟ (مثلاً: سعر الفسيخ، الفروع، أو الفرق بين أنواع الرنجة)"
  # ================== WEBHOOK ==================
