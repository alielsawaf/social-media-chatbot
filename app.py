...
def get_answer(text):
    ...
    # 1. الترحيب (بكل أشكاله)
    greetings = ["اهلا", "سلام", "مساء", "صباح", "مرحبا", "هاي", "ازيك", "ازيكم"]
    if any(word in q for word in greetings):
        return "أهلاً بك في رنجة أبو السيد 👋 نورتنا.. أساعد حضرتك ازاي؟"

    # 2. المنيو (الرابط المباشر) - لازم تكون فوق عشان ترد فوراً
    if "منيو" in q or "الكتالوج" in q or "الاسعار كلها" in q:
        return f"ده لينك منيو المنتجات بتاعتنا كاملة بالأسعار: {FAQ_MAP['منيو']}"

    # 3. كلمات التأكيد (اه / ياريت)
    if q in ["اه", "ياريت", "ايوه", "تمام", "ماشي", "قول", "ابعت", "اوك"]:
        return "من عيوني! قولي تحب تعرف أسعار (الرنجة) ولا (الفسيخ) ولا (الماكريل) ولا (التونة)؟"

    # 4. الكلمات الحساسة (دود - دم - فرق)
    ...
    # 7. قسم التونة (المنشأ والأسعار)
    if "تون" in q:
        if any(w in q for w in ["مصر", "مستورد", "نوع", "منين"]):
    ...
    # 8. الماكريل والبطارخ
    if "ماكريل" in q: return f"💰 أسعار الماكريل:\n- {PRODUCT_MAP['Gutted Smoked Mackerel Salted']}\n- {PRODUCT_MAP['Mackerel Fillets Vacuumed']}"
    if "بطارخ" in q: return f"💰 {PRODUCT_MAP['Herring Roe with Vegetable Oil']}\n\n{PRODUCT_MAP['Herring Roe White']}"

    # 9. طلب السعر العام (لو ملقاش صنف في الكلام)
    if any(w in q for w in ["سعر", "بكام", "اسعار"]):
        return "تحب تعرف أسعار أنهي صنف؟ (رنجة - فسيخ - ماكريل - تونة - بطارخ)"
