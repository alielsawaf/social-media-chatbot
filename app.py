...
def get_answer(text):
    ...
    # 2. قسم "ليه" و "ايه الفرق" (حل مشكلة الفيليه الناشفة والفرق بين 24)
    # حطينا القسم ده فوق السعر عشان لو العميل سأل عن "سبب" ميردش بـ "سعر"
    ...
    # 3. الكلمات الحساسة (دود - طفيليات)
    if "دود" in q or "طفيليات" in q: return FAQ_MAP["الرنجة فيها دود"]

    # 4. قسم البطارخ (بوري vs رنجة)
    if "بطارخ" in q:
        if any(w in q for w in ["بوري", "نشي", "نشو"]):
            return f"💰 {PRODUCT_MAP['Herring Roe White']}"
        return f"💰 أسعار بطارخ الرنجة:\n- {PRODUCT_MAP['Herring Roe with Vegetable Oil']}\n- {PRODUCT_MAP['Herring Roe with Orange Sauce']}"

    # 5. قسم الرنجة (فيليه - 24 - عادي)
    if "رنج" in q:
        if "فيليه" in q: return f"💰 {PRODUCT_MAP['Herring Fillets without Oil']}\n\nومتاح صوصات (كاري، فلفل، كافيار) بأسعار تبدأ من 150 ج."
        if "24" in q: return f"💰 {PRODUCT_MAP['Smoked Herring 24 Kerat']}"
        return f"💰 أسعار الرنجة:\n- {PRODUCT_MAP['Smoked Herring']}\n- {PRODUCT_MAP['Smoked Herring 24 Kerat']}\n\nتحب تعرف أسعار الفيليه والصوصات كمان؟"

    # 6. الفسيخ والماكريل والتونة والسردين
    if "فسيخ" in q: return f"💰 أسعار الفسيخ:\n- {PRODUCT_MAP['Salted Mullet without Bacteria']}\n- {PRODUCT_MAP['Salted Mullet with Roe']}"
    if "ماكريل" in q: return f"💰 {PRODUCT_MAP['Gutted Smoked Mackerel Salted']}\n\n{PRODUCT_MAP['Mackerel Fillets Vacuumed']}"
    if "تون" in q:
        if any(w in q for w in ["مصر", "مستورد", "نوع", "منين"]): return FAQ_MAP["التونة مستوردة ولا مصري"]
        return "💰 تونة أبو السيد مصري يلوفين، تبدأ من 70 ج. متاح (قطع - مفتتة - سبريد). تحب أبعتلك المنيو؟"
    if "سردين" in q: return "للأسف يا فندم السردين غير متاح حالياً، متوفر فسيخ بوري ممتاز، تحب أعرضلك أسعاره؟"

    # 7. حل مشكلة "اه"
    if q in ["اه", "ايوه", "ياريت", "تمام", "ماشي"]:
        return "تحت أمرك! قولي محتاج تعرف أسعار إيه بالظبط؟ أو اكتب 'المنيو' وهبعتلك الكتالوج فوراً."
