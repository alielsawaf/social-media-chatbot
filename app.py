...
def get_answer(text):
    ...
    if "دود" in q or "مدود" in q or "طفيليات" in q:
    ...
    if "رنج" in q:
        ...
        return (
            ...
            f"تحب أعرفك على أسعار الفيليه والصوصات كمان؟"
        )
    ...
    if "تون" in q or "تونه" in q:
        if any(w in q for w in ["ابيض", "احمر", "فرق", "الفرق"]):
            return FAQ_MAP["الفرق بين لحم التونة الابيض والاحمر"]
    ...
    if "سردين" in q:
        return "للأسف السردين غير متاح حالياً، متوفر فسيخ بوري ممتاز، تحب أعرضلك أسعاره؟"

    # 8. متابعة ذكية
    if q in ["اه", "ايوه", "ياريت"]:
        return "تمام 👌 تحب تعرف أسعار رنجة ولا فسيخ ولا بطارخ؟"
